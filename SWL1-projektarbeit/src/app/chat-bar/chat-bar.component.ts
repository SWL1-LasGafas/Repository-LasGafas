...
export class ChatBarComponent implements OnInit {
  ...
  sendChat() {
    ...
    this.nickName = this.pService.myNickname;
    this.tstamp = +this.pad(dt.getDay(),2)+'/'+this.pad((dt.getMonth()+1),2)+'/'+dt.getFullYear()+' - '+this.pad(dt.getHours(),2)+':'+this.pad(dt.getMinutes(),2); // Hier wird das Datum formatiert
